...
class ProsesPasien implements KontrakPasienPresenter
{
	...
	function prosesDataPasien()
	{
		try {
			...
			while ($row = $this->tabelpasien->getResult()) {
				...
				$pasien->setPhone($row['telp']); //mengisi gender
	...
	function prosesDataPasienById($id)
	{
		try {
			...
			// while ($row = $this->tabelpasien->getResult()) {
				//ambil hasil query
				$pasien = new Pasien($data['id'], $data['nik'], $data['nama'], $data['tempat'], $data['tl'], $data['gender'], $data['email'], $data['telp']); //instansiasi objek pasien untuk setiap data pasien
